  ...
  ->Belt.Array.map(((key, collideCircle)) =>
      <React.Fragment key>
        <div className="flex justify-between mt-3">
          ...
          <Button_UI onClick={_ => {modalDispatch(OpenModal(key, None))}}>
            {React.string("x")}
          </Button_UI>
          <ConfirmModal_Debug
            id=key
        ...
        <div className="grid grid-cols-12 gap-1 mb-3">
          ...
          {Belt.List.map(
             collideCircle.collisions, (collisionType: Type.collideType) =>
             switch (collisionType) {
             | Box(entity) =>
               <Entity_Debug
                 key=entity
                 entity={
                   "Box(" ++ Uuid_Util.humanFriendlyEntity(entity) ++ ")"
                 }
             ...
             | Circle(entity) =>
               <Entity_Debug
                 key=entity
                 entity={
                   "Circle(" ++ Uuid_Util.humanFriendlyEntity(entity) ++ ")"
                 }
                 humanFriendlyEntity=false
               />
             }
