...
let componentList: Belt.List.t(Select_UI.selectOption) = [
  ...
  {value: "TransformLocalPosition", text: "Transform.localPosition", disabled: false},
];

let componentEntitiesList = (appState: Type.state, component: Type.animatedComponent): Belt.List.t(Select_UI.selectOption) => {
  let selectOption = switch component {
  | FieldFloat(_) => 
    Belt.Map.String.mapWithKey(appState.fieldFloat, (key, _): Select_UI.selectOption => 
      { value: key, text: Uuid_Util.humanFriendlyEntity(key), disabled: false }
    )
  | FieldVector(_) => 
    Belt.Map.String.mapWithKey(appState.fieldVector, (key, _): Select_UI.selectOption => 
      { value: key, text: Uuid_Util.humanFriendlyEntity(key), disabled: false }
    )
  | TransformLocalPosition(_) => 
    Belt.Map.String.mapWithKey(appState.transform, (key, _): Select_UI.selectOption => 
      { value: key, text: Uuid_Util.humanFriendlyEntity(key), disabled: false }
    )
  // | _ => {
...
let setAnimationComponent = (
  ...
  let animatedComponent = switch component {
  | Type.FieldFloat(_) => Type.FieldFloat(componentEntity)
  | Type.FieldVector(_) => Type.FieldVector(componentEntity)
  | Type.TransformLocalPosition(_) => Type.TransformLocalPosition(componentEntity)
  };

  appDispatch(App_Context.SetAnimationComponent(animationEntity, animatedComponent));
...
let make = (~name: Type.entity) => {
  ...
  let modalBody = switch(animation) {
        ...
        <div className="col-span-6 grid grid-cols-12 gap-1">
          ...
          <div className="col-span-4">
              <Select_UI
                ...
                onChange={component => setAnimationComponent(
                  ~appDispatch,
          ...
          <div className="col-span-4">
              <Select_UI
                ...
                onChange={componentEntity => setAnimationComponent(
                  ~appDispatch, 
...
{React.string("ssss")}
      ...
      <div className="flex border-solid border border-black h-20 bg-gray-700 bg-opacity-75 relative overflow-hidden">
        {Belt.List.mapWithIndex(animation.keyframes, (index, keyframe: Type.keyframe) =>
